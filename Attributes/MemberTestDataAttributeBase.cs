...
: MemberDataAttributeBase
{
        ...
        DisposalTracker disposalTracker)
    {
        ...
        if (runtimeGenericType != typeof(ITheoryTestDataRow))
        {
            return theoryDataRowCollection;
        }
        ...
        {
            var ttdr = item as TheoryTestDataRow<ITestData>;

            if (ttdr!.TestDisplayName is null)
            {
                ttdrList.Add(ttdr);
            }
            else
            {
                ttdrList.Add(new TheoryTestDataRow<ITestData>(
                    ttdr,
                    ttdr.ArgsCode,
                    testMethodName));
            }
    ...
    {
        if (dataRow is TheoryTestDataRow<ITestData> theoryTestDataRow)
        {
